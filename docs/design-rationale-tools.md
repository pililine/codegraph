...
## 0. 结论先行：对"P1 — tools.ts 体积过大"的修正

初始项目分析（`docs/project-analysis.md`）将 tools.ts 列为 P1 重构问题，理由是"单文件行数多"。**这个判断是错误的，已在 `project-analysis.md` 中撤销。**

错误在于把**现象**（行数多）等同于**问题**（设计差）。行数本身不是有效的问题指标。有效的判断标准是：

| 维度 | 说明 | tools.ts 实际情况 |
|------|------|-----------------|
| **维护摩擦** | 一个改动需要修改多少不相关的地方？ | 低：每个工具是独立的私有方法，新增工具只需在 `execute()` 的 switch 中加一行 |
| **测试困难** | 行为是否可以通过公开接口自动验证？ | 低：5 个测试文件通过 `ToolHandler.execute()` 和 `getTools()` 覆盖；所有 handler 均私有，无需暴露 |
| **重复逻辑** | 同一逻辑是否在多处维护？ | 低：`synthEdgeNote`、`findAllSymbols`、`validateString` 等是统一的单一来源 |
| **隐式状态** | 是否存在跨调用的副作用或隐藏耦合？ | 基本无：唯一的隐式状态是 `projectCache`（跨项目连接池），有文档注释 |
| **认知负担** | 读者能否从结构中推断出职责？ | 中等：文件确实长，但每个方法命名清晰（`handleTrace`、`handleExplore`），可直接定位 |

**认知负担**是唯一有争议的维度，但它是文件大的**伴随成本**，不是设计问题。文件的内聚性是真实的，拆分不会降低认知负担，只会把需要跨文件理解的隐式耦合（共享 `synthEdgeNote`、共享 `fileCache` 语义）暴露成跨文件导入。

真正从 tools.ts 中识别出的三个具体问题（见第 6 节）是**内容层面**的问题，与文件大小无关：`markSessionConsulted` 的平台特定逻辑、`handleStatus` 的硬编码字符串、`looksLikeFeatureRequest` 的未验证启发式。
